...
\end{itemize}
Entrances are located on one side of the rectangular environment, and exits are
located on the opposite side; the number of entrances, exits and agents are
provided as parameters to the model.
In the areas where agents' paths cross, crowding occurs, with faster agents
attempting to pass slower agents and at times getting stuck.

Data assimilation is applied to improve the model estimates of agent positions
in the environment; as such, the state vector comprises of the coordinates of
the agents.
The ground truth for this experiment is taken to be synthetic data generated by
a separate run of the model.
Observations are then generated by adding normally distributed random noise to
the ground truth at each assimilation step.
